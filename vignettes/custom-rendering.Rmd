  ...
  columns = list(
    Price = colDef(
      ...
      footer = JS("function(colInfo) {
        const values = colInfo.data.map(function(row) {
...
cellInfoProps <- dplyr::tribble(
  ...
  "row", '{ Petal.Length: 1.7, Species: "setosa" }', "row data values",
  ...
  "subRows", '[{ Petal.Length: 1.7, Species: "setosa" }, ...]', "array of sub row data values (aggregated cells only)",
  "level", "0", "row nesting depth (zero-based)",
  "selected", "true", "whether the row is selected (new in v0.2.3.9000)"
)
...
colInfoProps <- dplyr::tribble(
  ~Property, ~Example, ~Description,
  "column", '{ id: "Petal.Length", name: "Petal Length", filterValue: "petal" }', "column info object",
  "data", '[{ Petal.Length: 1.7, Petal.Width: 0.2, _subRows: [] }, ...]', "sorted array of row data values in the table"
)
...
To render a custom filter input, provide an R function with up to 2 optional arguments:
...
colDef(
  filterInput = function(values, name) {
    ...
    #
    # output:
    #   - content to render (e.g. an HTML tag or HTML string)
...
colDef(
  filterInput = JS("
    function(column, state) {
      ...
      //
      // output:
      //  - content to render (e.g. an HTML string or React element)
